...
import type { DetailedDailyPlan, PlanMode } from '@/types/travelTypes'
import { isBadWeather } from '@/services/travelWeatherCore'
import { addDayToCalendar, copyDayReminderText } from '@/utils/travelReminders'
...
const isSimple = computed(() => props.planMode !== 'detailed')
const weather = computed(() => props.day.weather)
const badWeather = computed(() => weather.value ? isBadWeather(weather.value) : false)
const addingDay = ref(false)
const copyingDay = ref(false)

async function handleAddDayToCalendar() {
  if (addingDay.value) return
  const title = `${props.day.dateText || ''} ${props.day.title || props.destination || '旅游行程'}提醒`.trim()
  addingDay.value = true
  try {
    const result = await addDayToCalendar(props.day, title, props.remindBeforeMinutes ?? 15)
    if (result.ok && result.mode === 'shared') {
      ElMessage.success('已通过系统分享添加日历，请在弹窗中选择「日历」')
    } else if (!result.ok) {
      if (result.reason === 'empty') ElMessage.warning('当天暂无可添加的提醒')
      else if (result.reason !== 'cancelled') {
        ElMessage.warning(result.message || '添加当天日历失败，请复制提醒文本后手动添加')
      }
    }
  } catch {
    ElMessage.error('添加当天日历失败，请复制提醒文本后手动添加')
  } finally {
    addingDay.value = false
  }
}

async function handleCopyDayReminders() {
  if (copyingDay.value) return
  copyingDay.value = true
  try {
    await copyDayReminderText(props.day, props.remindBeforeMinutes ?? 15)
  } finally {
  ...
  <div class="day-card trip-day-card">
    <div class="day-head trip-day-header">
      <div class="day-head-main">
        ...
        <h4>{{ day.title }}</h4>
        <span v-if="day.dateText" class="day-date">{{ day.dateText }}</span>
      </div>
      <div class="day-head-actions">
        <el-button size="small" plain :loading="addingDay" @click="handleAddDayToCalendar">添加当天到日历</el-button>
        <el-button size="small" plain :loading="copyingDay" @click="handleCopyDayReminders">复制当天提醒</el-button>
      </div>
    </div>

    <div v-if="weather" class="weather-bar" :class="{ danger: badWeather }">
      <div class="weather-main">
        <span class="weather-label">天气</span>
        <strong>{{ weather.weather }} {{ weather.tempMin }}-{{ weather.tempMax }}℃</strong>
        <span v-if="weather.wind">{{ weather.wind }}</span>
        <span v-if="weather.precipitationProbability != null">降雨概率 {{ weather.precipitationProbability }}%</span>
      </div>
      <p class="weather-advice">出行建议：{{ weather.travelAdvice }}</p>
      <p v-if="badWeather && day.badWeatherAlternative" class="weather-alt">
        天气提醒：{{ day.badWeatherAlternative }}
      </p>
      <p v-else-if="badWeather && weather.warning" class="weather-alt">天气提醒：{{ weather.warning }}</p>
    </div>
    ...
    <p v-if="!isSimple || !day.timeline?.length" class="summary">{{ day.daySummary }}</p>

    <div v-if="!isSimple && day.transportSummary?.routeDescription" class="transport-box">
    ...
    <div v-if="(day.timeline || []).length" class="timeline">
      ...
      <div v-for="(item, i) in day.timeline" :key="i" class="tl-item trip-timeline-item">
        ...
        <div class="tl-body trip-timeline-content">
          <el-tag v-if="!isSimple" size="small" effect="plain">{{ typeLabel[item.type] || item.type }}</el-tag>
    ...
    <div v-if="!isSimple && (day.scenicSpots || []).length" class="spots-section">
      <h5>当日景区</h5>
      <DetailedScenicSpotCard v-for="(s, i) in day.scenicSpots" :key="i" :spot="s" />
    </div>

    <div v-if="!isSimple && (day.meals || []).length" class="meals">
      <h5>餐饮建议</h5>
    ...
    <div v-if="!isSimple && day.hotelSuggestion?.area" class="hotel">
    ...
    <div v-if="!isSimple && day.dayBudget?.dayTotal" class="day-budget">
    ...
    <div v-if="(day.tips || []).length" class="tips-block">
      <h5>今日提醒</h5>
      <ul class="tips"><li v-for="(t, i) in day.tips" :key="i">{{ t }}</li></ul>
    </div>
  </div>
</template>
...
.day-head { display: flex; align-items: flex-start; justify-content: space-between; gap: 12px; margin-bottom: 10px; }
.day-head-main { display: flex; flex-wrap: wrap; align-items: center; gap: 10px; }
...
.day-head-actions { display: flex; flex-wrap: wrap; gap: 8px; flex-shrink: 0; }
.weather-bar { margin: 0 20px 12px; padding: 12px 14px; border-radius: 14px; background: #f0f9ff; border: 1px solid #bae6fd; font-size: 13px; }
.weather-bar.danger { background: #fff7ed; border-color: #fdba74; }
.weather-main { display: flex; flex-wrap: wrap; align-items: center; gap: 8px; }
.weather-label { font-size: 12px; color: #0369a1; font-weight: 700; }
.weather-advice, .weather-alt { margin: 8px 0 0; line-height: 1.55; color: #475569; }
.weather-alt { color: #c2410c; font-weight: 600; }
...
.tips-block { margin: 0 20px 16px; }
.tips { margin: 0; padding-left: 18px; font-size: 12px; line-height: 1.55; }
