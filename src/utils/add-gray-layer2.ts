...
// 1️⃣  在 onAdd 中保存顶点数，后面要重复 draw
const quadVertexCount = 4;

// 2️⃣  render 回调中替换原先一次 draw 为循环 draw
import { mat4 } from 'gl-matrix'; // ✅ 引入矩阵工具
...
export function addGrayLayer(options: AddGrayLayerOptions) {
    ...
    let customLayer: mapboxgl.CustomLayerInterface & { visible?: boolean } = {
        id,
        type: 'custom',
        renderingMode: '2d',
        ...
        onAdd: function (map, glContext) {
            ...
            image.onload = () => {
                ...
                (customLayer as any).grayTexture = texture; // 手动保存纹理到 customLayer 实例上，便于后续访问
            };
            image.src = imageUrl;
         ...
         */
        render: function (_glContext, matrixParam) {
            ...
            // 世界宽度（单位：MercatorCoordinate.x，等于 1）
            const WORLD_WIDTH = 1.0;
            const OFFSETS = [-1, 0, 1];          // 渲染 3 个副本，足够覆盖任何视口

            OFFSETS.forEach(offset => {
                // 复制 Mapbox 传进来的矩阵，平移一整圈
                const m = mat4.clone(matrixParam);
                mat4.translate(m, m, [offset * WORLD_WIDTH, 0, 0]);
                gl.uniformMatrix4fv(uMatrix, false, m);

                // 其它 uniform（u_texture、u_colorRamp、u_opacity）保持不变
...
// 导出一个函数，用于初始化灰度图层图片
export function initGrayLayerImage (options: AddGrayLayerOptions) {
    // 定义一个透明的PNG图片
    const TRANSPARENT_PNG = 
    'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJ' +
    'AAAADUlEQVQImWNgYGBgAAAABQABDQottAAAAABJRU5ErkJggg=='

    // 调用addGrayLayer函数，传入options和TRANSPARENT_PNG
    addGrayLayer({...options, imageUrl: TRANSPARENT_PNG})

    // 如果options中有map属性，则添加一个自定义的TDT矢量图层
    options.map?.addSource('customTdtVecSource', {
        type: 'raster',
        tiles: [
      ...
      options.map?.addLayer({
        id: 'customTdtVecLayer',
        type: 'raster',
        source: 'customTdtVecSource',
        layout: {
          visibility: 'visible',
        },
        paint: {
...
/**
 * 更新灰度图层的 imageUrl，不重建图层，直接在 WebGL 纹理中替换图像数据
 */
// 导出一个函数，用于更新灰度图层图像
export function updateGrayLayerImage(
    // 地图对象
    map: mapboxgl.Map,
    // 图层ID
    id: string,
    ...
    // 获取图层对象
    const raw = map.getLayer(id) as any;
    // 获取图层实现对象
    const impl = raw.implementation as any;
    // 获取WebGL渲染上下文
    const gl: WebGLRenderingContext = impl.gl;
    // 获取灰度纹理
    const grayTex: WebGLTexture = impl.grayTexture;
    // 如果WebGL渲染上下文或灰度纹理不存在，则输出警告信息并返回
    if (!gl || !grayTex) {
      console.warn(`灰度图层 ${id} 或其纹理不存在`);
      return;
    }
  ...
  // 导出一个函数，用于隐藏mapboxgl地图中的灰层
  export function hideGrayLayer(map: mapboxgl.Map, id: string) {
    // 获取地图中的灰层
    const raw = map.getLayer(id) as any;
    // 获取灰层的实现
    const impl = raw?.implementation as any;
  ...
  // 导出一个函数，用于显示灰度图层
  export function showGrayLayer(map: mapboxgl.Map, id: string) {
    // 获取指定id的图层
    const raw = map.getLayer(id) as any;
    // 获取图层的实现
    const impl = raw?.implementation as any;
